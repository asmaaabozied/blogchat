...
@section('title', 'Create New Reply')
@section('index-url', route('reply.index'))
@section('store-url', route('reply.store'))
@section('new-form')
    ...
    <x-input-long-text rows="2" cols="10" name="content" :value="old('content')" title="Reply"></x-input-long-text>

    <div class="form-group">
        <label for="type">Type: </label>
        <select name="repliable_type" id="repliable_type" class="selectpicker form-control" onchange="change()">
            <option {{old('repliable_type') == \App\Models\Comment::class ? 'selected':''}} value="{{\App\Models\Comment::class}}">Comment</option>
            <option {{old('repliable_type') == \App\Models\Reply::class ? 'selected':''}} value="{{\App\Models\Reply::class}}">Reply</option>
        </select>
    </div>
    ...
    <div class="form-group" id="comment_id">
        ...
        <select name="repliable_id" id="comment_id" class="selectpicker form-control">
            @foreach(\App\Models\Comment::all() as $item)
                <option {{old('repliable_id') == $item['id'] ? 'selected':''}} value="{{$item['id']}}">{{$item['id']}} - {{$item->user->name}}</option>
            @endforeach
        </select>
    </div>
